...
from .utils.der import hexFromInt, parse, DerFieldType, encodeConstructed, encodedPrimitive
...
class PublicKey:
    ...
    def toDer(self):
        hexadecimal = encodeConstructed(
            encodeConstructed(
                encodedPrimitive(DerFieldType.object, _ecdsaPublicKeyOid),
                encodedPrimitive(DerFieldType.object, self.curve.oid),
            ),
            encodedPrimitive(DerFieldType.bitString, self.toString(encoded=True)),
        )
