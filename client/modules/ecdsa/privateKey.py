...
from .utils.der import hexFromInt, parse, encodeConstructed, DerFieldType, encodedPrimitive
...
class PrivateKey:
    ...
    def toDer(self):
        ...
        hexadecimal = encodeConstructed(
            encodedPrimitive(DerFieldType.integer, 1),
            encodedPrimitive(DerFieldType.octetString, hexFromInt(self.secret)),
            encodedPrimitive(DerFieldType.oidContainer, encodedPrimitive(DerFieldType.object, self.curve.oid)),
            encodedPrimitive(DerFieldType.publicKeyPointContainer, encodedPrimitive(DerFieldType.bitString, publicKeyString))
        )
