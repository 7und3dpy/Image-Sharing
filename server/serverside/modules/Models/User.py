...
class UserModel: 
    ...
    def generate_token(self):
        return secrets.token_bytes(self.__token_length)
    ...
    def get_user_public_key(self, user_id):
        '''Get user Public key and key length
        ...
        Output: 
            ...
            - key_length: user_id's key length


        '''
        db_exec = self.__db.execute(
            'SELECT public_key, key_length FROM user where id = ?', (user_id, )
        )
    ...
    def check_api_token(self, user_id, api_token):
        ...
        db_exec = self.__db.execute(
            'SELECT * FROM user WHERE (id = ?) AND (api_token = ?)', (user_id, api_token)
        )
    ...
    def check_public_key_exist(self, public_key, key_length): 
        '''
        Check if the public_key existed
