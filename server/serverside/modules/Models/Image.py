...
class ImageModel: 
    ...
    def check_img_exist(self, user_id, img_id):
        '''
        Check if img_id exist (as known as user_id has permissions with it)
    ...
    def get_img_filename(self, user_id, img_id): 
        ...
        Input: 
             - user_id: User ID (has permission)
             - img_id: Image ID

        Return: 
            - image_location: Filename on server
            - image_realname: Original Filename
        '''

        db_exec = self.__db.execute(
            'SELECT images.location, images.real_name FROM images, sharing WHERE sharing.user_id = ? AND sharing.image_id = ? AND sharing.image_id = image_id', 
            (user_id, img_id)
        )
